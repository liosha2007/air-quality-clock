...
int Keyestudio_CCS811::begin(void) {
    uint8_t id = 0;
    Wire.begin();
    softReset();
    delay(100);
    if (readReg(CCS811_REG_HW_ID, &id, 1) != 1) {
        DBG("");
...
uint16_t Keyestudio_CCS811::readBaseLine() {
    uint8_t buffer[2];
    readReg(CCS811_REG_BASELINE, buffer, 2);
    return buffer[0] << 8 | buffer[1];
}

void Keyestudio_CCS811::writeBaseLine(uint16_t baseLine) {
    uint8_t buffer[2];
...
void Keyestudio_CCS811::setMeasurementMode(uint8_t thresh, uint8_t interrupt, eDRIVE_MODE_t mode) {
    uint8_t measurement[1] = {0};
    measurement[0] = (thresh << 2) | (interrupt << 3) | (mode << 4);
    writeReg(CCS811_REG_MEAS_MODE, measurement, 1);
}

void Keyestudio_CCS811::setMeasCycle(eCycle_t cycle) {
    uint8_t measurement[1] = {0};
    measurement[0] = cycle << 4;
    writeReg(CCS811_REG_MEAS_MODE, measurement, 1);
}

uint8_t Keyestudio_CCS811::getMeasurementMode() {
    uint8_t meas[1] = {0};
    readReg(CCS811_REG_MEAS_MODE, meas, 1);
    return meas[0];
}

void Keyestudio_CCS811::setThresholds(uint16_t lowToMed, uint16_t medToHigh) {
...
uint16_t Keyestudio_CCS811::getCO2PPM() {
    uint8_t buffer[8];
...
uint16_t Keyestudio_CCS811::getTVOCPPB() {
    uint8_t buffer[8];
    readReg(CCS811_REG_ALG_RESULT_DATA, buffer, 8);
...
void Keyestudio_CCS811::setInTempHum(float temperature, float humidity)    // compensate for temperature and relative humidity
{
    int _temp, _rh;
    if (temperature > 0)
    ...
    _temp = _temp +
            25;  // temperature high byte is stored as T+25°C in the sensor's memory so the value of byte is positive
...
void Keyestudio_CCS811::writeReg(uint8_t reg, const void *pBuf, size_t size) {
    if (pBuf == NULL) {
...
uint8_t Keyestudio_CCS811::readReg(uint8_t reg, const void *pBuf, size_t size) {
    if (pBuf == NULL) {
        DBG("pBuf ERROR!! : null pointer");
    }
    uint8_t *_pBuf = (uint8_t *) pBuf;
